...
%%
    for j = 1:length(dat.valid_frame)
        if(dat.valid_frame(j))
            fprintf('Image %d of %d for Test ID %d\n',j, length(dat.images), test_subject_id(i));
            error = zeros(17,1);
...
save([net_base filesep net_path{n,1} filesep 'mpii_3dhp_prediction_' int2str(net_path{n,2}) '.mat'], 'sequencewise_per_joint_error', 'sequencewise_activity_labels');
[seq_table, activity_table] = mpii_evaluate_errors(sequencewise_per_joint_error, sequencewise_activity_labels);

out_file = [net_base filesep net_path{n,1} filesep 'mpii_3dhp_evaluation_' int2str(net_path{n,2})];
